...
// board/ 까지만 입력 받아도 첫 번째 page로 이동되도록
router.get("/",(req,res)=>
{
...
router.post('/write',(req,res)=>{
    let fn = async function()
    {
        ...
        user_seq = req.session.userSeq;  // 로그인 된 user의 고유 번호 저장
        board_writer = req.session.userName;  // 로그인 된 user의 이름 저장
        board_title = req.body.board_title;  // 게시판에서 작성된 제목을 저장
        board_contents = req.body.board_contents;  // 게시판에서 작성된 내용을 저장
        try
        {
...
router.get('/read/:board_seq',(req,res)=>
{
    ...
    let fn = async function()
    {
        ...
        try
        {
            ...
            // 게시글을 작성한 user의 고유 번호를 얻기 위해 유저 정보 조회하여 user_info 값에 저장
            // user 정보를 얻는 이유는 로그인 된 user가 자신이 작성한 글을 읽는지 타인이 작성한 글을 읽는지 구분하기 위해
...
router.post('/update',(req,res)=>{
    let fn = async function()
    {
        ...
        board_seq = req.body.board_seq;  // 게시글 고유 번호를 저장
        board_title = req.body.board_title;  // 수정된 게시글 제목을 저장
        board_contents = req.body.board_contents;  // 수정된 게시글 본문 내용을 저장
        try
        {
...
router.post('/delete',(req,res)=>{
    let fn = async function()
    {
        ...
        board_seq = req.body.board_seq;  // 게시글의 고유 번호 저장
        try
        {
...
router.post('/reply_write',(req,res)=>{
    let fn = async function()
    {
        ...
        user_seq = req.session.userSeq;  // 로그인된 User의 고유 번호를 저장
        board_seq = req.body.board_seq;  // 게시글 고유 번호를 저장
        board_reply_writer = req.session.userName;  // 로그인 된 User의 이름을 저장
        board_reply_contents = req.body.board_reply_contents;  // 입력된 댓글 내용을 저장
        try
        {
